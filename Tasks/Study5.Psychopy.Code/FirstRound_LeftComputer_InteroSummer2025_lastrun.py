...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='G:\\My Drive\\InteroSummer2025\\Task\\FirstRound_LeftComputer_InteroSummer2025_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    Crit_Ac = ""
    Crit_De = ""
    port_startexp = parallel.ParallelPort(address='0xCFF8')
    ...
    valenceImage = visual.ImageStim(
        ...
        ori=0.0, pos=(0, 0), draggable=False, size=(1.0, 0.307),
    ...
    Circle = visual.ShapeStim(
        ...
        lineWidth=1.0,
        colorSpace='rgb', lineColor=[-1.0000, -1.0000, 0.0902], fillColor=[-1.0000, -1.0000, 1.0000],
        opacity=None, depth=0.0, interpolate=True)
    port_starttrial = parallel.ParallelPort(address='0xCFF8')
    ...
    speedImage = visual.ImageStim(
        ...
        ori=0.0, pos=(0, 0), draggable=False, size=(1,.483),
        color=[1,1,1], colorSpace='rgb', opacity=None,
        flipHoriz=False, flipVert=False,
        texRes=128.0, interpolate=True, depth=-1.0)
    port_teststop = parallel.ParallelPort(address='0xCFF8')
    ...
    TestCircle = visual.ShapeStim(
        ...
        colorSpace='rgb', lineColor=[-1.0000, -1.0000, 0.0902], fillColor=[-1.0000, -1.0000, 1.0000],
        opacity=None, depth=0.0, interpolate=True)
    port_teststart = parallel.ParallelPort(address='0xCFF8')
    ...
    countScreen = visual.TextStim(win=win, name='countScreen',
        ...
        languageStyle='LTR',
        depth=-1.0);
    port_HRstart = parallel.ParallelPort(address='0xCFF8')
    ...
    port_HRstop = parallel.ParallelPort(address='0xCFF8')
    ...
    text_6 = visual.TextStim(win=win, name='text_6',
        ...
        languageStyle='LTR',
        depth=-1.0);
    port_stopexp = parallel.ParallelPort(address='0xCFF8')
    ...
    # create an object to store info about Routine StartupCode
    StartupCode = data.Routine(
        name='StartupCode',
        components=[port_startexp],
    )
    StartupCode.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    # store start times for StartupCode
    StartupCode.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    StartupCode.tStart = globalClock.getTime(format='float')
    StartupCode.status = STARTED
    thisExp.addData('StartupCode.started', StartupCode.tStart)
    StartupCode.maxDuration = None
    # keep track of which components have finished
    StartupCodeComponents = StartupCode.components
    for thisComponent in StartupCode.components:
    ...
    StartupCode.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine and routineTimer.getTime() < 1.0:
        ...
        if port_startexp.status == NOT_STARTED and t >= 0.0-frameTolerance:
            ...
            port_startexp.status = STARTED
            win.callOnFlip(port_startexp.setData, int(16))
        ...
        if not continueRoutine:  # a component has requested a forced-end of Routine
            StartupCode.forceEnded = routineForceEnded = True
            break
    ...
    # store stop times for StartupCode
    StartupCode.tStop = globalClock.getTime(format='float')
    StartupCode.tStopRefresh = tThisFlipGlobal
    thisExp.addData('StartupCode.stopped', StartupCode.tStop)
    if port_startexp.status == STARTED:
        win.callOnFlip(port_startexp.setData, int(0))
    # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
    if StartupCode.maxDurationReached:
        routineTimer.addTime(-StartupCode.maxDuration)
    elif StartupCode.forceEnded:
        routineTimer.reset()
    else:
    ...
    # create an object to store info about Routine judgeValence
    judgeValence = data.Routine(
        name='judgeValence',
        components=[valenceImage, valenceSlider],
    )
    judgeValence.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    valenceSlider.reset()
    ...
    # store start times for judgeValence
    judgeValence.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    judgeValence.tStart = globalClock.getTime(format='float')
    judgeValence.status = STARTED
    thisExp.addData('judgeValence.started', judgeValence.tStart)
    judgeValence.maxDuration = None
    # keep track of which components have finished
    judgeValenceComponents = judgeValence.components
    for thisComponent in judgeValence.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    judgeValence.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            judgeValence.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in judgeValence.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    for thisComponent in judgeValence.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for judgeValence
    judgeValence.tStop = globalClock.getTime(format='float')
    judgeValence.tStopRefresh = tThisFlipGlobal
    thisExp.addData('judgeValence.stopped', judgeValence.tStop)
    ...
    # create an object to store info about Routine judgearousal_2
    judgearousal_2 = data.Routine(
        name='judgearousal_2',
        components=[arousalImage, arousalSlider],
    )
    judgearousal_2.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    arousalSlider.reset()
    ...
    # store start times for judgearousal_2
    judgearousal_2.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    judgearousal_2.tStart = globalClock.getTime(format='float')
    judgearousal_2.status = STARTED
    thisExp.addData('judgearousal_2.started', judgearousal_2.tStart)
    judgearousal_2.maxDuration = None
    # keep track of which components have finished
    judgearousal_2Components = judgearousal_2.components
    for thisComponent in judgearousal_2.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    judgearousal_2.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            judgearousal_2.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in judgearousal_2.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    for thisComponent in judgearousal_2.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for judgearousal_2
    judgearousal_2.tStop = globalClock.getTime(format='float')
    judgearousal_2.tStopRefresh = tThisFlipGlobal
    thisExp.addData('judgearousal_2.stopped', judgearousal_2.tStop)
    ...
    # create an object to store info about Routine Instruct1
    Instruct1 = data.Routine(
        name='Instruct1',
        components=[text_7, key_resp_4],
    )
    Instruct1.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    ...
    key_resp_4.rt = []
    _key_resp_4_allKeys = []
    # store start times for Instruct1
    Instruct1.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    Instruct1.tStart = globalClock.getTime(format='float')
    Instruct1.status = STARTED
    thisExp.addData('Instruct1.started', Instruct1.tStart)
    Instruct1.maxDuration = None
    # keep track of which components have finished
    Instruct1Components = Instruct1.components
    for thisComponent in Instruct1.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    Instruct1.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            Instruct1.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in Instruct1.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    for thisComponent in Instruct1.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for Instruct1
    Instruct1.tStop = globalClock.getTime(format='float')
    Instruct1.tStopRefresh = tThisFlipGlobal
    thisExp.addData('Instruct1.stopped', Instruct1.tStop)
    # check responses
    ...
    # create an object to store info about Routine Instruct2
    Instruct2 = data.Routine(
        name='Instruct2',
        components=[text_3, key_resp_3],
    )
    Instruct2.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    ...
    key_resp_3.rt = []
    _key_resp_3_allKeys = []
    # store start times for Instruct2
    Instruct2.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    Instruct2.tStart = globalClock.getTime(format='float')
    Instruct2.status = STARTED
    thisExp.addData('Instruct2.started', Instruct2.tStart)
    Instruct2.maxDuration = None
    # keep track of which components have finished
    Instruct2Components = Instruct2.components
    for thisComponent in Instruct2.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    Instruct2.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            Instruct2.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in Instruct2.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    for thisComponent in Instruct2.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for Instruct2
    Instruct2.tStop = globalClock.getTime(format='float')
    Instruct2.tStopRefresh = tThisFlipGlobal
    thisExp.addData('Instruct2.stopped', Instruct2.tStop)
    # check responses
    ...
    for level, condition in trials:
        ...
        trial = data.Routine(
            name='trial',
            components=[Circle, port_starttrial],
        )
        ...
        while continueRoutine:
            ...
            if port_starttrial.status == NOT_STARTED and t >= 0.0-frameTolerance:
                ...
                port_starttrial.status = STARTED
                win.callOnFlip(port_starttrial.setData, int(48))
        ...
        # create an object to store info about Routine judgebreath
        judgebreath = data.Routine(
            name='judgebreath',
            components=[kbChange, speedImage, port_teststop],
        )
        judgebreath.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        ...
        event.clearEvents('keyboard')
        # store start times for judgebreath
        judgebreath.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        judgebreath.tStart = globalClock.getTime(format='float')
        judgebreath.status = STARTED
        thisExp.addData('judgebreath.started', judgebreath.tStart)
        judgebreath.maxDuration = None
        # keep track of which components have finished
        judgebreathComponents = judgebreath.components
        for thisComponent in judgebreath.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        if isinstance(trials, data.TrialHandler2) and thisTrial.thisN != trials.thisTrial.thisN:
            continueRoutine = False
        judgebreath.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine:
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
            ...
            if port_teststop.status == NOT_STARTED and t >= 0.0-frameTolerance:
                ...
                port_teststop.status = STARTED
                win.callOnFlip(port_teststop.setData, int(64))
            ...
            # check if all components have finished
            if not continueRoutine:  # a component has requested a forced-end of Routine
                judgebreath.forceEnded = routineForceEnded = True
                break
            continueRoutine = False  # will revert to True if at least one component still running
            for thisComponent in judgebreath.components:
                if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
        ...
        for thisComponent in judgebreath.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for judgebreath
        judgebreath.tStop = globalClock.getTime(format='float')
        judgebreath.tStopRefresh = tThisFlipGlobal
        thisExp.addData('judgebreath.stopped', judgebreath.tStop)
        # check responses
        ...
        # create an object to store info about Routine judgeconfidence
        judgeconfidence = data.Routine(
            name='judgeconfidence',
            components=[confidenceImage, confidenceSlider],
        )
        judgeconfidence.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        ...
        # store start times for judgeconfidence
        judgeconfidence.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        judgeconfidence.tStart = globalClock.getTime(format='float')
        judgeconfidence.status = STARTED
        thisExp.addData('judgeconfidence.started', judgeconfidence.tStart)
        judgeconfidence.maxDuration = None
        # keep track of which components have finished
        judgeconfidenceComponents = judgeconfidence.components
        for thisComponent in judgeconfidence.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        if isinstance(trials, data.TrialHandler2) and thisTrial.thisN != trials.thisTrial.thisN:
            continueRoutine = False
        judgeconfidence.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine:
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
            ...
            # check if all components have finished
            if not continueRoutine:  # a component has requested a forced-end of Routine
                judgeconfidence.forceEnded = routineForceEnded = True
                break
            continueRoutine = False  # will revert to True if at least one component still running
            for thisComponent in judgeconfidence.components:
                if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
        ...
        for thisComponent in judgeconfidence.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for judgeconfidence
        judgeconfidence.tStop = globalClock.getTime(format='float')
        judgeconfidence.tStopRefresh = tThisFlipGlobal
        thisExp.addData('judgeconfidence.stopped', judgeconfidence.tStop)
    ...
    # create an object to store info about Routine CalcThresholds
    CalcThresholds = data.Routine(
        name='CalcThresholds',
        components=[],
    )
    CalcThresholds.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    # store start times for CalcThresholds
    CalcThresholds.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    CalcThresholds.tStart = globalClock.getTime(format='float')
    CalcThresholds.status = STARTED
    thisExp.addData('CalcThresholds.started', CalcThresholds.tStart)
    CalcThresholds.maxDuration = None
    # keep track of which components have finished
    CalcThresholdsComponents = CalcThresholds.components
    for thisComponent in CalcThresholds.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    CalcThresholds.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            CalcThresholds.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in CalcThresholds.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    for thisComponent in CalcThresholds.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for CalcThresholds
    CalcThresholds.tStop = globalClock.getTime(format='float')
    CalcThresholds.tStopRefresh = tThisFlipGlobal
    thisExp.addData('CalcThresholds.stopped', CalcThresholds.tStop)
    ...
    # create an object to store info about Routine TestTrialInstruct
    TestTrialInstruct = data.Routine(
        name='TestTrialInstruct',
        components=[text_2, key_resp_2],
    )
    TestTrialInstruct.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    ...
    key_resp_2.rt = []
    _key_resp_2_allKeys = []
    # store start times for TestTrialInstruct
    TestTrialInstruct.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    TestTrialInstruct.tStart = globalClock.getTime(format='float')
    TestTrialInstruct.status = STARTED
    thisExp.addData('TestTrialInstruct.started', TestTrialInstruct.tStart)
    TestTrialInstruct.maxDuration = None
    # keep track of which components have finished
    TestTrialInstructComponents = TestTrialInstruct.components
    for thisComponent in TestTrialInstruct.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    TestTrialInstruct.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            TestTrialInstruct.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in TestTrialInstruct.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    for thisComponent in TestTrialInstruct.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for TestTrialInstruct
    TestTrialInstruct.tStop = globalClock.getTime(format='float')
    TestTrialInstruct.tStopRefresh = tThisFlipGlobal
    thisExp.addData('TestTrialInstruct.stopped', TestTrialInstruct.tStop)
    # check responses
    ...
    for thisTestTrial in testTrials:
        ...
        TestTrial = data.Routine(
            name='TestTrial',
            components=[TestCircle, port_teststart],
        )
        ...
        while continueRoutine:
            ...
            if port_teststart.status == NOT_STARTED and t >= 0.0-frameTolerance:
                ...
                port_teststart.status = STARTED
                win.callOnFlip(port_teststart.setData, int(80))
    ...
    # create an object to store info about Routine CompleteInstruct
    CompleteInstruct = data.Routine(
        name='CompleteInstruct',
        components=[key_space, text_4],
    )
    CompleteInstruct.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    ...
    key_space.rt = []
    _key_space_allKeys = []
    # store start times for CompleteInstruct
    CompleteInstruct.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    CompleteInstruct.tStart = globalClock.getTime(format='float')
    CompleteInstruct.status = STARTED
    thisExp.addData('CompleteInstruct.started', CompleteInstruct.tStart)
    CompleteInstruct.maxDuration = None
    # keep track of which components have finished
    CompleteInstructComponents = CompleteInstruct.components
    for thisComponent in CompleteInstruct.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    CompleteInstruct.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            CompleteInstruct.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in CompleteInstruct.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    for thisComponent in CompleteInstruct.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for CompleteInstruct
    CompleteInstruct.tStop = globalClock.getTime(format='float')
    CompleteInstruct.tStopRefresh = tThisFlipGlobal
    thisExp.addData('CompleteInstruct.stopped', CompleteInstruct.tStop)
    # check responses
    ...
    # create an object to store info about Routine HeartInstruct
    HeartInstruct = data.Routine(
        name='HeartInstruct',
        components=[key_space_2, text_5],
    )
    HeartInstruct.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    # create starting attributes for key_space_2
    key_space_2.keys = []
    key_space_2.rt = []
    _key_space_2_allKeys = []
    # store start times for HeartInstruct
    HeartInstruct.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    HeartInstruct.tStart = globalClock.getTime(format='float')
    HeartInstruct.status = STARTED
    thisExp.addData('HeartInstruct.started', HeartInstruct.tStart)
    HeartInstruct.maxDuration = None
    # keep track of which components have finished
    HeartInstructComponents = HeartInstruct.components
    for thisComponent in HeartInstruct.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    HeartInstruct.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            HeartInstruct.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in HeartInstruct.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    for thisComponent in HeartInstruct.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for HeartInstruct
    HeartInstruct.tStop = globalClock.getTime(format='float')
    HeartInstruct.tStopRefresh = tThisFlipGlobal
    thisExp.addData('HeartInstruct.stopped', HeartInstruct.tStop)
    # check responses
    ...
    # set up handler to look after randomisation of conditions etc
    HeartbeatLoop = data.TrialHandler2(
        name='HeartbeatLoop',
        nReps=1.0, 
        method='random', 
        extraInfo=expInfo, 
    ...
    for thisHeartbeatLoop in HeartbeatLoop:
        ...
        # create an object to store info about Routine HeartCount
        HeartCount = data.Routine(
            name='HeartCount',
            components=[readyScreen, countScreen, port_HRstart],
        )
        HeartCount.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        # store start times for HeartCount
        HeartCount.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        HeartCount.tStart = globalClock.getTime(format='float')
        HeartCount.status = STARTED
        thisExp.addData('HeartCount.started', HeartCount.tStart)
        HeartCount.maxDuration = None
        # keep track of which components have finished
        HeartCountComponents = HeartCount.components
        for thisComponent in HeartCount.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # if trial has changed, end Routine now
        if isinstance(HeartbeatLoop, data.TrialHandler2) and thisHeartbeatLoop.thisN != HeartbeatLoop.thisTrial.thisN:
            continueRoutine = False
        HeartCount.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine:
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
            ...
            if port_HRstart.status == NOT_STARTED and t >= 5-frameTolerance:
                ...
                port_HRstart.status = STARTED
                win.callOnFlip(port_HRstart.setData, int(112))
            ...
            # check if all components have finished
            if not continueRoutine:  # a component has requested a forced-end of Routine
                HeartCount.forceEnded = routineForceEnded = True
                break
            continueRoutine = False  # will revert to True if at least one component still running
            for thisComponent in HeartCount.components:
                if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
        ...
        for thisComponent in HeartCount.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for HeartCount
        HeartCount.tStop = globalClock.getTime(format='float')
        HeartCount.tStopRefresh = tThisFlipGlobal
        thisExp.addData('HeartCount.stopped', HeartCount.tStop)
        if port_HRstart.status == STARTED:
            win.callOnFlip(port_HRstart.setData, int(0))
        ...
        # create an object to store info about Routine HeartReport
        HeartReport = data.Routine(
            name='HeartReport',
            components=[beatReport, HRenter_text, port_HRstop],
        )
        HeartReport.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        HRenter_text.reset()
        ...
        # store start times for HeartReport
        HeartReport.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        HeartReport.tStart = globalClock.getTime(format='float')
        HeartReport.status = STARTED
        thisExp.addData('HeartReport.started', HeartReport.tStart)
        HeartReport.maxDuration = None
        # keep track of which components have finished
        HeartReportComponents = HeartReport.components
        for thisComponent in HeartReport.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        if isinstance(HeartbeatLoop, data.TrialHandler2) and thisHeartbeatLoop.thisN != HeartbeatLoop.thisTrial.thisN:
            continueRoutine = False
        HeartReport.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine:
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
            ...
            if port_HRstop.status == NOT_STARTED and t >= 0.0-frameTolerance:
                ...
                port_HRstop.status = STARTED
                win.callOnFlip(port_HRstop.setData, int(128))
            ...
            # check if all components have finished
            if not continueRoutine:  # a component has requested a forced-end of Routine
                HeartReport.forceEnded = routineForceEnded = True
                break
            continueRoutine = False  # will revert to True if at least one component still running
            for thisComponent in HeartReport.components:
                if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
        ...
        for thisComponent in HeartReport.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for HeartReport
        HeartReport.tStop = globalClock.getTime(format='float')
        HeartReport.tStopRefresh = tThisFlipGlobal
        thisExp.addData('HeartReport.stopped', HeartReport.tStop)
    ...
    # create an object to store info about Routine EndPart1
    EndPart1 = data.Routine(
        name='EndPart1',
        components=[key_space_3, text_6, port_stopexp],
    )
    EndPart1.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    # create starting attributes for key_space_3
    key_space_3.keys = []
    key_space_3.rt = []
    _key_space_3_allKeys = []
    # store start times for EndPart1
    EndPart1.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    EndPart1.tStart = globalClock.getTime(format='float')
    EndPart1.status = STARTED
    thisExp.addData('EndPart1.started', EndPart1.tStart)
    EndPart1.maxDuration = None
    # keep track of which components have finished
    EndPart1Components = EndPart1.components
    for thisComponent in EndPart1.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    EndPart1.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        if port_stopexp.status == NOT_STARTED and t >= 0.0-frameTolerance:
            ...
            port_stopexp.status = STARTED
            win.callOnFlip(port_stopexp.setData, int(32))
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            EndPart1.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in EndPart1.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    for thisComponent in EndPart1.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for EndPart1
    EndPart1.tStop = globalClock.getTime(format='float')
    EndPart1.tStopRefresh = tThisFlipGlobal
    thisExp.addData('EndPart1.stopped', EndPart1.tStop)
    # check responses
