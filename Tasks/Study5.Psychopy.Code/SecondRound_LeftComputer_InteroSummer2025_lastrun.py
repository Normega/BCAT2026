...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='G:\\My Drive\\InteroSummer2025\\Task\\SecondRound_LeftComputer_InteroSummer2025_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    text_4 = visual.TextStim(win=win, name='text_4',
        text="Great! You've completed the circle task. Please alert the researcher that you have finished.\n\nPress <spacebar> to continue.",
        font='Arial',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color=textColor, colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=-1.0);
    port_taskcomplete = parallel.ParallelPort(address='0xCFF8')
    ...
    CompleteInstruct = data.Routine(
        ...
        components=[key_space, text_4, port_taskcomplete],
    )
    ...
    while continueRoutine:
        ...
        if text_4.status == STARTED:
            # update params
            pass
        # *port_taskcomplete* updates
        
        # if port_taskcomplete is starting this frame...
        if port_taskcomplete.status == NOT_STARTED and t >= 0-frameTolerance:
            # keep track of start time/frame for later
            port_taskcomplete.frameNStart = frameN  # exact frame index
            port_taskcomplete.tStart = t  # local t and not account for scr refresh
            port_taskcomplete.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(port_taskcomplete, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.addData('port_taskcomplete.started', t)
            # update status
            port_taskcomplete.status = STARTED
            port_taskcomplete.status = STARTED
            win.callOnFlip(port_taskcomplete.setData, int(32))
        
        # if port_taskcomplete is stopping this frame...
        if port_taskcomplete.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > port_taskcomplete.tStartRefresh + 1.0-frameTolerance:
                # keep track of stop time/frame for later
                port_taskcomplete.tStop = t  # not accounting for scr refresh
                port_taskcomplete.tStopRefresh = tThisFlipGlobal  # on global time
                port_taskcomplete.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.addData('port_taskcomplete.stopped', t)
                # update status
                port_taskcomplete.status = FINISHED
                win.callOnFlip(port_taskcomplete.setData, int(0))
