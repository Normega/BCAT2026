...
#
# Sections:
#   S8.1  Data preparation — z-scores, session-long pivot
#   S8.2  Partial correlations: MAIA → confidence (per session)
#   S8.3  Partial correlations: MAIA → threshold (overall + 4 conditions)
#   S8.4  Multilevel model: confidence ~ MAIA + MSES + condition
#           (a) visual-first group only — clean within-person design
#           (b) full sample — session + condition as covariates
#   S8.5  Multilevel model: threshold ~ MAIA + MSES + Salience + Direction
#
# DESIGN NOTE:
...
#   full sample. The visual-first subgroup (S8.4a) provides the
...
#
# Outputs:
#   s8_partial_correlations.csv
#   s8_threshold_partial_correlations.csv
#   s8_multilevel_confidence_results.csv
#   s8_multilevel_threshold_results.csv
# ============================================================

message("\n========================================")
message("S8: MAIA SPECIFICITY — SELF-ESTEEM CONTROLS")
message("========================================")


# ============================================================
# S8.1  Data Preparation
# ============================================================
...
# ============================================================
# S8.2  Partial Correlations: MAIA → Confidence
#
...
s8_conf_partials <- dplyr::bind_rows(conf_partial_rows)
print(s8_conf_partials |>
        dplyr::filter(predictor == "MAIA_total_z") |>
        dplyr::select(label, control, r, p, n))


# ============================================================
# S8.3  Partial Correlations: MAIA → Threshold
#
...
s8_thresh_partials <- dplyr::bind_rows(thresh_partial_rows)
cat("\nMAIA_total_z → thresh_breath_mean (all control sets):\n")
print(s8_thresh_partials |>
        dplyr::filter(predictor == "MAIA_total_z",
...
message("\n--- S8.4: Multilevel models: Confidence ~ MAIA + MSES + Condition ---")
...
# ── S8.4a: Visual-first group (within-person, clean design) ──────────

message("\n  S8.4a: Visual-first group only")
...
message("\n--- S8.5: Multilevel model: Threshold ~ MAIA + MSES ---")
...
readr::write_csv(s8_conf_partials,
                 paste0(RESULTS_DIR, "s8_partial_correlations.csv"))
readr::write_csv(s8_thresh_partials,
                 paste0(RESULTS_DIR, "s8_threshold_partial_correlations.csv"))
readr::write_csv(s8_conf_mlm,
                 paste0(RESULTS_DIR, "s8_multilevel_confidence_results.csv"))
readr::write_csv(s8_thresh_mlm,
                 paste0(RESULTS_DIR, "s8_multilevel_threshold_results.csv"))

message("Saved: s8_partial_correlations.csv")
message("Saved: s8_threshold_partial_correlations.csv")
message("Saved: s8_multilevel_confidence_results.csv")
message("Saved: s8_multilevel_threshold_results.csv")
message("analysis_s8_maia_selfesteem.R complete.")
